// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.harness.platform;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Export;
import com.pulumi.core.annotations.ResourceType;
import com.pulumi.core.internal.Codegen;
import com.pulumi.harness.Utilities;
import com.pulumi.harness.platform.ConnectorJenkinsArgs;
import com.pulumi.harness.platform.inputs.ConnectorJenkinsState;
import com.pulumi.harness.platform.outputs.ConnectorJenkinsAuth;
import java.lang.String;
import java.util.List;
import java.util.Optional;
import javax.annotation.Nullable;

/**
 * Resource for creating a Jenkins connector.
 * 
 * ## Example Usage
 * ```java
 * package generated_program;
 * 
 * import com.pulumi.Context;
 * import com.pulumi.Pulumi;
 * import com.pulumi.core.Output;
 * import com.pulumi.harness.platform.ConnectorJenkins;
 * import com.pulumi.harness.platform.ConnectorJenkinsArgs;
 * import com.pulumi.harness.platform.inputs.ConnectorJenkinsAuthArgs;
 * import com.pulumi.harness.platform.inputs.ConnectorJenkinsAuthJenkinsBearerTokenArgs;
 * import java.util.List;
 * import java.util.ArrayList;
 * import java.util.Map;
 * import java.io.File;
 * import java.nio.file.Files;
 * import java.nio.file.Paths;
 * 
 * public class App {
 *     public static void main(String[] args) {
 *         Pulumi.run(App::stack);
 *     }
 * 
 *     public static void stack(Context ctx) {
 *         var test = new ConnectorJenkins(&#34;test&#34;, ConnectorJenkinsArgs.builder()        
 *             .auth(ConnectorJenkinsAuthArgs.builder()
 *                 .jenkinsBearerToken(ConnectorJenkinsAuthJenkinsBearerTokenArgs.builder()
 *                     .tokenRef(String.format(&#34;account.%s&#34;, harness_platform_secret_text.test().id()))
 *                     .build())
 *                 .type(&#34;Bearer Token(HTTP Header)&#34;)
 *                 .build())
 *             .delegateSelectors(&#34;harness-delegate&#34;)
 *             .description(&#34;test&#34;)
 *             .identifier(&#34;identifier&#34;)
 *             .jenkinsUrl(&#34;https://jenkinss.com/&#34;)
 *             .tags(&#34;foo:bar&#34;)
 *             .build());
 * 
 *     }
 * }
 * ```
 * 
 * ## Import
 * 
 * Import account level jenkins connector
 * 
 * ```sh
 *  $ pulumi import harness:platform/connectorJenkins:ConnectorJenkins example &lt;connector_id&gt;
 * ```
 * 
 *  Import org level jenkins connector
 * 
 * ```sh
 *  $ pulumi import harness:platform/connectorJenkins:ConnectorJenkins example &lt;ord_id&gt;/&lt;connector_id&gt;
 * ```
 * 
 *  Import project level jenkins connector
 * 
 * ```sh
 *  $ pulumi import harness:platform/connectorJenkins:ConnectorJenkins example &lt;org_id&gt;/&lt;project_id&gt;/&lt;connector_id&gt;
 * ```
 * 
 */
@ResourceType(type="harness:platform/connectorJenkins:ConnectorJenkins")
public class ConnectorJenkins extends com.pulumi.resources.CustomResource {
    /**
     * This entity contains the details for Jenkins Authentication.
     * 
     */
    @Export(name="auth", refs={ConnectorJenkinsAuth.class}, tree="[0]")
    private Output</* @Nullable */ ConnectorJenkinsAuth> auth;

    /**
     * @return This entity contains the details for Jenkins Authentication.
     * 
     */
    public Output<Optional<ConnectorJenkinsAuth>> auth() {
        return Codegen.optional(this.auth);
    }
    /**
     * Tags to filter delegates for connection.
     * 
     */
    @Export(name="delegateSelectors", refs={List.class,String.class}, tree="[0,1]")
    private Output</* @Nullable */ List<String>> delegateSelectors;

    /**
     * @return Tags to filter delegates for connection.
     * 
     */
    public Output<Optional<List<String>>> delegateSelectors() {
        return Codegen.optional(this.delegateSelectors);
    }
    /**
     * Description of the resource.
     * 
     */
    @Export(name="description", refs={String.class}, tree="[0]")
    private Output</* @Nullable */ String> description;

    /**
     * @return Description of the resource.
     * 
     */
    public Output<Optional<String>> description() {
        return Codegen.optional(this.description);
    }
    /**
     * Unique identifier of the resource.
     * 
     */
    @Export(name="identifier", refs={String.class}, tree="[0]")
    private Output<String> identifier;

    /**
     * @return Unique identifier of the resource.
     * 
     */
    public Output<String> identifier() {
        return this.identifier;
    }
    /**
     * Jenkins Url.
     * 
     */
    @Export(name="jenkinsUrl", refs={String.class}, tree="[0]")
    private Output<String> jenkinsUrl;

    /**
     * @return Jenkins Url.
     * 
     */
    public Output<String> jenkinsUrl() {
        return this.jenkinsUrl;
    }
    /**
     * Name of the resource.
     * 
     */
    @Export(name="name", refs={String.class}, tree="[0]")
    private Output<String> name;

    /**
     * @return Name of the resource.
     * 
     */
    public Output<String> name() {
        return this.name;
    }
    /**
     * Unique identifier of the organization.
     * 
     */
    @Export(name="orgId", refs={String.class}, tree="[0]")
    private Output</* @Nullable */ String> orgId;

    /**
     * @return Unique identifier of the organization.
     * 
     */
    public Output<Optional<String>> orgId() {
        return Codegen.optional(this.orgId);
    }
    /**
     * Unique identifier of the project.
     * 
     */
    @Export(name="projectId", refs={String.class}, tree="[0]")
    private Output</* @Nullable */ String> projectId;

    /**
     * @return Unique identifier of the project.
     * 
     */
    public Output<Optional<String>> projectId() {
        return Codegen.optional(this.projectId);
    }
    /**
     * Tags to associate with the resource.
     * 
     */
    @Export(name="tags", refs={List.class,String.class}, tree="[0,1]")
    private Output</* @Nullable */ List<String>> tags;

    /**
     * @return Tags to associate with the resource.
     * 
     */
    public Output<Optional<List<String>>> tags() {
        return Codegen.optional(this.tags);
    }

    /**
     *
     * @param name The _unique_ name of the resulting resource.
     */
    public ConnectorJenkins(String name) {
        this(name, ConnectorJenkinsArgs.Empty);
    }
    /**
     *
     * @param name The _unique_ name of the resulting resource.
     * @param args The arguments to use to populate this resource's properties.
     */
    public ConnectorJenkins(String name, ConnectorJenkinsArgs args) {
        this(name, args, null);
    }
    /**
     *
     * @param name The _unique_ name of the resulting resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param options A bag of options that control this resource's behavior.
     */
    public ConnectorJenkins(String name, ConnectorJenkinsArgs args, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        super("harness:platform/connectorJenkins:ConnectorJenkins", name, args == null ? ConnectorJenkinsArgs.Empty : args, makeResourceOptions(options, Codegen.empty()));
    }

    private ConnectorJenkins(String name, Output<String> id, @Nullable ConnectorJenkinsState state, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        super("harness:platform/connectorJenkins:ConnectorJenkins", name, state, makeResourceOptions(options, id));
    }

    private static com.pulumi.resources.CustomResourceOptions makeResourceOptions(@Nullable com.pulumi.resources.CustomResourceOptions options, @Nullable Output<String> id) {
        var defaultOptions = com.pulumi.resources.CustomResourceOptions.builder()
            .version(Utilities.getVersion())
            .build();
        return com.pulumi.resources.CustomResourceOptions.merge(defaultOptions, options, id);
    }

    /**
     * Get an existing Host resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state
     * @param options Optional settings to control the behavior of the CustomResource.
     */
    public static ConnectorJenkins get(String name, Output<String> id, @Nullable ConnectorJenkinsState state, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        return new ConnectorJenkins(name, id, state, options);
    }
}
